...
using UnnamedCoin.Bitcoin.Features.Wallet.Helpers;
using UnnamedCoin.Bitcoin.Features.Wallet.Interfaces;
using UnnamedCoin.Bitcoin.Features.Wallet.Models;
using UnnamedCoin.Bitcoin.Utilities;
using UnnamedCoin.Bitcoin.Utilities.JsonErrors;
using UnnamedCoin.Bitcoin.Utilities.ModelStateErrors;

namespace UnnamedCoin.Bitcoin.Features.Wallet.Controllers
{
    /// <summary>
    ...
    public class WalletController : Controller
    {
        ...
        readonly IWalletTransactionHandler walletTransactionHandler;
        ...
        /// </summary>
        /// <param name="language">
        ///     The language for the words in the mnemonic. The options are: English, French, Spanish, Japanese,
        ///     ChineseSimplified and ChineseTraditional. Defaults to English.
        /// </param>
        ...
        public IActionResult Create([FromBody] WalletCreationRequest request)
        {
            ...
            try
            {
                var requestMnemonic = string.IsNullOrEmpty(request.Mnemonic) ? null : new Mnemonic(request.Mnemonic);

                var mnemonic = this.walletManager.CreateWallet(request.Password, request.Name, request.Passphrase,
                    requestMnemonic);
        ...
        public IActionResult SignMessage([FromBody] SignMessageRequest request)
        {
            ...
            try
            {
                var signature = this.walletManager.SignMessage(request.Password, request.WalletName,
        ...
        public IActionResult Load([FromBody] WalletLoadRequest request)
        {
            ...
            try
            {
                var wallet = this.walletManager.LoadWallet(request.Password, request.Name);
                return Ok();
            }
        ...
        public IActionResult RecoverViaExtPubKey([FromBody] WalletExtPubRecoveryRequest request)
        {
            ...
            try
            {
                this.walletManager.RecoverWallet(request.Name, ExtPubKey.Parse(request.ExtPubKey), request.AccountIndex,  request.CreationDate);
        ...
        public IActionResult GetGeneralInfo([FromQuery] WalletName request)
        {
            ...
            try
            {
                ...
                var (folder, fileNameCollection) = this.walletManager.GetWalletsFiles();
                var searchFile = Path.ChangeExtension(request.Name, this.walletManager.GetWalletFileExtension());
                var fileName = fileNameCollection.FirstOrDefault(i => i.Equals(searchFile));
        ...
        public IActionResult GetHistory([FromQuery] WalletHistoryRequest request)
        {
            ...
            try
            {
                ...
                var accountsHistory = this.walletManager.GetHistory(request.WalletName, request.AccountName);
                ...
                foreach (var accountHistory in accountsHistory)
                {
                    ...
                    var query = accountHistory.History;
                    ...
                    var history = items.Where(t =>
                            !t.Address.IsChangeAddress() || t.Address.IsChangeAddress() && t.Transaction.IsSpent())
                        .ToList();

                    // Represents a sublist of 'change' transactions.
                    var allchange = items.Where(t => t.Address.IsChangeAddress()).ToList();

                    foreach (var item in history)
                    {
                        // Count only unique transactions and limit it to MaxHistoryItemsPerAccount.
                        var processedTransactions = uniqueProcessedTxIds.Count;
                        if (processedTransactions >= MaxHistoryItemsPerAccount) break;
                            ...
                            transaction.SpendingDetails.IsCoinStake.Value)
                        {
                            ...
                            var relatedOutputs = items.Where(h =>
                                h.Transaction.Id == transaction.SpendingDetails.TransactionId &&
                            ...
                            if (transaction.IsCoinStake != null) continue;
                        }
                        ...
                        if (transaction.SpendingDetails != null && transaction.SpendingDetails.IsCoinStake == null)
                        {
                            ...
                            var changeAddress =
                                allchange.FirstOrDefault(a => a.Transaction.Id == spendingTransactionId);
                            ...
                            var inputsAmount = new Money(spendingDetails
                                .Where(t => t.Transaction.SpendingDetails.TransactionId == spendingTransactionId)
                                .Sum(t => t.Transaction.Amount));

                            // The fee is calculated as follows: funds in utxo - amount spent - amount sent as change.
                            sentItem.Fee = inputsAmount - sentItem.Amount -
                                           (changeAddress == null ? 0 : changeAddress.Transaction.Amount);
                        ...
                        if (transaction.IsCoinStake != null && transaction.IsCoinStake.Value &&
                            transaction.SpendingDetails == null) continue;
                        ...
                        if (transaction.IsCoinStake == null && !address.IsChangeAddress())
                        {
                            ...
                            var existingReceivedItem =
                                FindSimilarReceivedTransactionOutput(transactionItems, transaction);
                    ...
                    var itemsToInclude = transactionItems.OrderByDescending(t => t.Timestamp)
        ...
        public IActionResult GetBalance([FromQuery] WalletBalanceRequest request)
        {
            ...
            try
            {
                ...
                var balances = this.walletManager.GetBalances(request.WalletName, request.AccountName);
                ...
                foreach (var balance in balances)
                {
                    ...
                    model.AccountsBalances.Add(new AccountBalanceModel
                    {
                        ...
                        {
                            var (confirmedAmount, unConfirmedAmount) = address.GetBalances();
        ...
        /// </summary>
        /// <param name="request">
        ///     An object containing the parameters used to retrieve the balance
        ///     at a specific wallet address.
        /// </param>
        ...
        /// </summary>
        /// <param name="request">
        ///     An object containing the parameters used to retrieve the
        ///     maximum spendable balance on an account.
        /// </param>
        /// <returns>
        ///     A JSON object containing the maximum spendable balance for an account
        ///     along with the fee required to spend it.
        /// </returns>
        [Route("maxbalance")]
        [HttpGet]
        public IActionResult GetMaximumSpendableBalance([FromQuery] WalletMaximumBalanceRequest request)
        {
            ...
            try
            {
                var transactionResult = this.walletTransactionHandler.GetMaximumSpendableAmount(
        ...
        /// </summary>
        /// <param name="request">
        ///     An object containing the parameters used to retrieve the spendable
        ...
        public IActionResult GetSpendableTransactions([FromQuery] SpendableTransactionsRequest request)
        {
            ...
            try
            {
                var spendableTransactions = this.walletManager.GetSpendableTransactionsInAccount(
        ...
        /// </summary>
        /// <param name="request">
        ///     An object containing the parameters used to estimate the fee
        ...
        public IActionResult GetTransactionFeeEstimate([FromBody] TxFeeEstimateRequest request)
        {
            ...
            try
            {
                ...
                foreach (var recipientModel in request.Recipients)
        ...
        /// <returns>
        ///     A JSON object including the transaction ID, the hex used to execute
        ///     the transaction, and the transaction fee.
        /// </returns>
        ...
        public IActionResult BuildTransaction([FromBody] BuildTransactionRequest request)
        {
            ...
            try
            {
                ...
                {
                    var wallet = this.walletManager.GetWallet(request.WalletName);
                    var account = wallet.GetAccount(request.AccountName);
                    if (account == null)
                ...
                if (!string.IsNullOrEmpty(request.FeeType)) context.FeeType = FeeParser.Parse(request.FeeType);
        ...
        public IActionResult SendTransaction([FromBody] SendTransactionRequest request)
        {
            ...
            try
            {
                var transaction = this.network.CreateTransaction(request.Hex);
        ...
        /// </summary>
        /// <returns>
        ///     A JSON object containing the wallet folder path and
        ///     the names of the files found within the folder.
        /// </returns>
        [Route("files")]
        [HttpGet]
        ...
        /// <returns>
        ///     A JSON object containing the name of the new account or an existing account
        ...
        /// </summary>
        /// <param name="request">
        ///     An object containing the necessary parameters to retrieve an
        ///     unused address for a wallet account.
        /// </param>
            ...
            try
            {
                var result =
                    this.walletManager.GetUnusedAddress(new WalletAccountReference(request.WalletName,
                        request.AccountName));
                return Json(request.Segwit ? result.Bech32Address : result.Address);
            }
            catch (Exception e)
            {
                this.logger.LogError("Exception occurred: {0}", e.ToString());
        ...
        ///     to the wallet).
        ///     <param name="request">
        ///         An object containing the necessary parameters to retrieve
        ///         unused addresses for a wallet account.
        ///     </param>
        ...
        public IActionResult GetUnusedAddresses([FromQuery] GetUnusedAddressesModel request)
        {
            ...
            try
            {
                ...
                return Json(result.Select(x => request.Segwit ? x.Bech32Address : x.Address).ToArray());
            }
            catch (Exception e)
            {
                this.logger.LogError("Exception occurred: {0}", e.ToString());
        ...
        ///     <param name="request">
        ///         An object containing the necessary parameters to retrieve
        ///         all addresses for a wallet account.
        ///     </param>
        ...
        ///     <param name="request">
        ///         An object containing the necessary parameters to remove transactions
        ///         from a wallet. The includes several options for specifying the transactions to remove.
        ///     </param>
        ///     <returns>
        ///         A JSON object containing all removed transactions identified by their
        ...
        public IActionResult RemoveTransactions([FromQuery] RemoveTransactionsModel request)
        {
            ...
            try
            {
                ...
                {
                    var ids = request.TransactionsIds.Select(uint256.Parse);
                ...
                if (result.Any() && request.ReSync)
                {
                    ...
                    var earliestDate = result.Min(r => r.creationTime);
                    var chainedHeader =
                ...
                var model = result.Select(r => new RemovedTransactionModel
                {
        ...
        public IActionResult GetExtPubKey([FromQuery] GetExtPubKeyModel request)
        {
            ...
            {
                var result =
                    this.walletManager.GetExtPubKey(new WalletAccountReference(request.WalletName,
                        request.AccountName));
                return Json(result);
            }
            catch (Exception e)
        ...
        public IActionResult Sync([FromBody] HashModel model)
        {
            ...
            var block = this.chainIndexer.GetHeader(uint256.Parse(model.Hash));
        ...
        public IActionResult SplitCoins([FromBody] SplitCoinsRequest request)
        {
            ...
            try
            {
                ...
                var address = this.walletManager.GetUnusedAddress(walletReference);

                Money totalAmount = request.TotalAmountToSplit;
                var singleUtxoAmount = totalAmount / request.UtxosCount;

                var recipients = new List<Recipient>(request.UtxosCount);
                for (var i = 0; i < request.UtxosCount; i++)
                    recipients.Add(new Recipient
                    {
                        ScriptPubKey = new BitcoinWitPubKeyAddress(address.Bech32Address, this.network).ScriptPubKey,
                        Amount = singleUtxoAmount
                    });
        ...
        public IActionResult DistributeUtxos([FromBody] DistributeUtxosRequest request)
        {
            ...
            try
            {
                ...
                if (request.Outpoints != null && request.Outpoints.Any())
                {
                    ...
                    foreach (var unspentOutputReference in spendableTransactions)
                ...
                var totalOutpointCount = spendableTransactions.Count();
                var calculatedTransactionCount = request.UtxosCount / request.UtxoPerTransaction;
                var inputsPerTransaction = totalOutpointCount / calculatedTransactionCount;
                ...
                if (calculatedTransactionCount > totalOutpointCount)
                {
                    ...
                    return ErrorHelpers.BuildErrorResponse(HttpStatusCode.BadRequest, "Invalid parameters",
                        "Invalid parameters");
                }
                ...
                for (var i = 0; i < request.UtxosCount; i++)
                {
                    ...
                    if ((i + 1) % request.UtxoPerTransaction == 0 || i == request.UtxosCount - 1)
                    {
                        ...
                        if (i == request.UtxosCount - 1)
                            foreach (var unspentOutputReference in spendableTransactions.Skip(
                        ...
                        var transferAmount = transactionTransferAmount / recipients.Count;
        ...
        void SyncFromBestHeightForRecoveredWallets(DateTime walletCreationDate)
        {
            ...
            var blockHeightToSyncFrom = this.chainIndexer.GetHeightAtTime(walletCreationDate);
            var currentSyncingHeight = this.walletSyncManager.WalletTip.Height;

            if (blockHeightToSyncFrom < currentSyncingHeight)
                this.walletSyncManager.SyncFromHeight(blockHeightToSyncFrom);
        }

        TransactionItemModel FindSimilarReceivedTransactionOutput(List<TransactionItemModel> items,
            TransactionData transaction)
        {
            var existingTransaction = items.FirstOrDefault(i => i.Id == transaction.Id &&

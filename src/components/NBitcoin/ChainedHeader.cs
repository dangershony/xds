...
namespace NBitcoin
{
    ...
    public enum BlockDataAvailabilityState
    {
        ...
        /// <summary>
        ///     We are interested in downloading the <see cref="Block" /> that is being represented by the current
    ...
    public class ChainedHeader
    {
        ...
        ChainedHeader(BlockHeader header, uint256 headerHash)
        {
        ...
        /// <summary>
        ///     An indicator that the current instance of <see cref="ChainedHeader" /> has been disconnected from the previous
        ///     instance.
        /// </summary>
        ...
        /// <summary>
        ///     Block represented by this header is assumed to be valid and only subset of validation rules should be applied to
        ///     it.
        ...
        /// <summary>
        ///     A pointer to the block data if available (this can be <c>null</c>), its availability will be represented by
        ///     <see cref="BlockDataAvailability" />.
        /// </summary>
        public Block Block { get; set; }

        /// <summary>
        ///     Points to the next <see cref="ChainedHeader" />, if a new branch of the chain is presented there can be more then
        ///     one <see cref="Next" /> header.
        /// </summary>
        public List<ChainedHeader> Next { get; }
        ...
        {
            var target = this.Header.Bits.ToBigInteger();
        ...
        public BlockLocator GetLocator()
        {
            ...
            while (pindex != null)
            {
                ...
                var height = Math.Max(pindex.Height - nStep, 0);
        ...
        public override string ToString()
        {
            return this.Height + "-" + this.HashBlock + "-" + this.BlockValidationState +
                   (this.Header is ProvenBlockHeader ? " - PH" : string.Empty);
        }
        ...
        /// <remarks>
        ///     This method compares the hash of the block header at the same height in the current chain
        ///     to verify the correct chained block header has been found.
        /// </remarks>
        public ChainedHeader FindAncestorOrSelf(ChainedHeader chainedHeader)
        {
            var found = GetAncestor(chainedHeader.Height);
        ...
        public Target GetWorkRequired(IConsensus consensus)
        {
            ...
            var proofOfWorkLimit = consensus.PowLimit;
            var lastBlock = this.Previous;
            var height = this.Height;

            if (lastBlock == null)
                return proofOfWorkLimit;

            var difficultyAdjustmentInterval = GetDifficultyAdjustmentInterval(consensus);
            ...
            if (height % difficultyAdjustmentInterval != 0)
            {
                if (consensus.PowAllowMinDifficultyBlocks)
                {
                    ...
                    if (this.Header.BlockTime > lastBlock.Header.BlockTime +
                        TimeSpan.FromTicks(consensus.PowTargetSpacing.Ticks * 2))
                        return proofOfWorkLimit;

                    // Return the last non-special-min-difficulty-rules-block.
                    var chainedHeader = lastBlock;
                    while (chainedHeader.Previous != null && chainedHeader.Height % difficultyAdjustmentInterval != 0 &&
                           chainedHeader.Header.Bits == proofOfWorkLimit)
            ...
            var pastHeight = lastBlock.Height - (difficultyAdjustmentInterval - 1);

            var firstChainedHeader = GetAncestor((int) pastHeight);
            ...
            var actualTimespan = lastBlock.Header.BlockTime - firstChainedHeader.Header.BlockTime;
            ...
            // Retarget.
            var newTarget = lastBlock.Header.Bits.ToBigInteger();
        ...
        long GetDifficultyAdjustmentInterval(IConsensus consensus)
        {
        ...
        public DateTimeOffset GetMedianTimePast()
        {
            ...
            var begin = MedianTimeSpan;
            var end = MedianTimeSpan;

            var chainedHeader = this;
            for (var i = 0; i < MedianTimeSpan && chainedHeader != null; i++, chainedHeader = chainedHeader.Previous)
        ...
        public bool Validate(Network network)
        {
            ...
            var genesisCorrect = this.Height != 0 || this.HashBlock == network.GetGenesis().GetHash();
        ...
        public ChainedHeader GetAncestor(int ancestorHeight)
        {
            ...
            while (walk != null && walk.Height != ancestorHeight)
            {
                ...
                var heightSkip = walk.Skip.Height;
                var heightSkipPrev = GetSkipHeight(walk.Height - 1);
                var skipAboveTarget = heightSkip > ancestorHeight;
                var skipPreviousBetterThanPreviousSkip =
                    !(heightSkipPrev < heightSkip - 2 && heightSkipPrev >= ancestorHeight);
